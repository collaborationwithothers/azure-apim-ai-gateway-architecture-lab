# Scenario: Auth and Identity

## 1. Business problem

Enterprise teams need Entra ID JWT validation, subscription keys, products, managed identity, Key Vault-backed named values, and per-application access control. Without a gateway pattern, each client implements these controls differently and backend governance becomes inconsistent.
