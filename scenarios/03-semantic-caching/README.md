# Scenario: Semantic Caching

## 1. Business problem

Enterprise teams need semantic cache lookup and store, Redis-compatible cache, tenant-aware partitioning, TTL, similarity threshold, and cache risk. Without a gateway pattern, each client implements these controls differently and backend governance becomes inconsistent.
...
Good candidates include FAQ prompts, documentation Q&A, repeated support questions, and non-personalized responses. Bad candidates include legal advice, medical advice, user-specific financial advice, volatile data, and prompts with strong authorization context.
...
Semantic caching can reduce latency and token cost, but it can return stale or semantically incorrect answers if cache design is weak.
