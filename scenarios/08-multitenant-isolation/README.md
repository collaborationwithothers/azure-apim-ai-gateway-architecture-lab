# Scenario: Multitenant Isolation

## 1. Business problem

Enterprise teams need tenant identification, subscription keys, products, quotas, cache partitions, logging dimensions, and backend routing. Without a gateway pattern, each client implements these controls differently and backend governance becomes inconsistent.
...
| Per-environment APIM | Strong environment separation | Clean Dev, Test, and Prod boundaries | Higher cost |
| Per-tenant APIM | Strongest isolation | Useful for regulated workloads | Expensive and operationally heavy |
